...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const regionsButton = document.querySelector('.regions-toggle-button');
  const dropdown = regionsWrapper ? regionsWrapper.querySelector('.search-dropdown') : null;
  ...
  parentItems.forEach(parentItem => {
    ...
    subitems.forEach(subitem => {
      subitem.addEventListener('click', function(e) {
        ...
        const buttonTextNode = Array.from(regionsButton.childNodes).find(node => node.nodeType === Node.TEXT_NODE);
        if (buttonTextNode) {
